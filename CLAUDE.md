...
**i18n flow**: every user-visible string in `index.html` carries `data-i18n="some_key"`. `translatePage(lang)` walks `[data-i18n]` and rewrites the element from `translations[lang][key]`. **Critical**: it uses `textContent` by default and only uses `innerHTML` for keys in the `HTML_KEYS` allow-list (currently `hero_desc`, `vision_text_2`). Adding `<br>`, `&mdash;`, or any HTML entity to a translation value for a non-whitelisted key will render as literal text. Either use a literal Unicode character (e.g. `—`, `•`) or add the key to `HTML_KEYS`. HTML default text in `index.html` is the EN fallback during the JS-loading flash — keep it in sync with the EN entry in `translations.js`. Sibling pages (`privacy.html`, etc.) are **statically translated** via separate files; they don't use `data-i18n`.
...
**Vision section**: `initLogisticsScene()` builds a Three.js scene (truck, cargo containers on a curve weaving through them, route tube, particle trail) into `#logisticsCanvas`. Container bodies use `MeshPhysicalMaterial` with clearcoat (chrome-gold), other parts use `MeshStandardMaterial`. All materials live in a module-scope `mats` object with `THEME.dark` / `THEME.light` palette tables; a `MutationObserver` on `<html data-theme>` calls `applyTheme()` which mutates materials in place via `material.setValues({...})` (NOT `Object.assign` — that overwrites the `Color` instance with a raw hex). Lazy — boots on `IntersectionObserver` first-intersection, skips heavy geometry on `(max-width: 1024px)` or `prefers-reduced-motion`.

**Contact form** (`#contactForm`): wired to Formspree at `https://formspree.io/f/mrejrnao` (action attribute). Inputs carry `name=` attributes (`company`, `email`, `message`) plus hidden `_subject`, `_replyto`, and `_gotcha` (honeypot) fields. Submit handler in `app.js` section 7 detects whether the action URL ends with the literal `FORMSPREE_ID` placeholder; if it does, it skips `fetch()` and routes to a **mailto fallback** (`openMailto()` opens the user's email client with the message pre-filled at `info@vortway.lt`). If Formspree fetch fails (offline, rate-limit, 5xx), it also falls back to mailto after 1.2s. Validation runs before submit: empty trim + email regex. Toast strings live in 7 locales as `toast_success`, `toast_error`, `toast_validation`, `toast_email_invalid`, `toast_mailto_opening`.
...
- **`translatePage` uses `textContent` by default** (see i18n flow above). Translations with `<br>`, `&mdash;`, or any HTML entity will render literally unless the key is in the `HTML_KEYS` allow-list. Use a Unicode character (`—`, `•`) or whitelist the key.
- **Three.js `material.setValues({color: 0xD4AF37})` is correct**; `Object.assign(material, {color: 0xD4AF37})` is NOT — it overwrites the `Color` instance with a raw number and then any subsequent `material.color.setHex(...)` throws. Always use `setValues` for theme switching.
- **`scrollIntoView` inside the modal scrolls the page background, not just the modal** — use `.modal-body.scrollTo({...})` directly. The body-scroll lock at modal-open prevents drift but `scrollIntoView` ignores it.
...
- **`og-image.png` is referenced but missing** — every LinkedIn / WhatsApp share of `vortway.lt` is currently broken. Pre-launch: generate a 1200×630 PNG at the project root before going live.
- **Formspree endpoint ID lives in `index.html` form `action`** — if it's ever rotated, change exactly one string. The JS auto-detects the `FORMSPREE_ID` placeholder and switches to mailto-only mode without code changes.

## Deployment

The site deploys to **Hostinger** at `vortway.lt`. There is no build step — upload the raw files. Production drops everything in this folder EXCEPT `.claude/`, `.playwright-mcp/`, `.git/`, `.gitignore`, `CLAUDE.md`, and any root-level `*.png` test artifacts (`check-*.png`, `logistics-*.png`, `smoke-*.png`). First Formspree submission to a new endpoint triggers a one-time *"verify your email"* email to the registered account (currently `info@vortway.lt`) — click that link or submissions silently drop.
